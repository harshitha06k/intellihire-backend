...
async def run_interview_turn(session: Session, answer: str) -> dict:
    ...
    if session.question_history and answer.strip():
        ...
        session.scores.technical = round(
            session.scores.technical * 0.6 +
            tech_score * 10 * 0.4,
            1
        )

        if session.scores.confidence < 5:
            session.scores.confidence = infer_confidence_from_answer(
                answer,
                tech_score
            )
        else:
            inferred = infer_confidence_from_answer(
                answer,
                tech_score
            )

            session.scores.confidence = round(
                session.scores.confidence * 0.7 +
                inferred * 0.3,
                1
            )
        ...
        if session.scores.engagement < 5:
            ...
            session.scores.engagement = depth_map.get(
                last_q.depth,
                50
            )
